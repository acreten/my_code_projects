...
def combat(weapon):
    ...
    if choice == '1':
        if weapon == "dagger":
            ...
            print_sleep(
                f"As the {enemy} moves to attack, you unsheath your new sword.", 2)
            print_sleep(
                f"The Sword of Ogoroth shines brightly in your hand as you brace yourself for the attack.",
                3)
            print_sleep(
                f"But the {enemy} takes one look at your shiny new toy and runs away!", 3)
            print_sleep(
...
def cave():
    ...
    if cave_visited:
        ...
        print_sleep(
            f"You discard your silly old {weapon} and take the sword with you.", 2)
        weapon = "sword"
    cave_visited = True
